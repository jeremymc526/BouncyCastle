...
static int ahci_host_init(struct ahci_uc_priv *uc_priv)
{
	...
	/*
	 * Determine which Ports are implemented by the iprop_ahsata,
	...
	for (i = 0; i < uc_priv->n_ports; i++) {
		...
		/* Ensure that the iprop_ahsata is in idle state */
...
static int ahci_port_start(struct ahci_uc_priv *uc_priv, u8 port)
{
	...
	speed = (port_status & SATA_PORT_SSTS_SPD_MASK) >> 4;
	if (speed == 1)
		speed_s = "1.5";
	else if (speed == 2)
		speed_s = "3.0";
...
static void iprop_ahsata_print_info(struct blk_desc *pdev)
{
...
static void iprop_ahsata_identify(struct ahci_uc_priv *uc_priv, u16 *id)
{
	struct sata_fis_h2d h2d __aligned(ARCH_DMA_MINALIGN);
	struct sata_fis_h2d *cfis = &h2d;
...
static void iprop_ahsata_xfer_mode(struct ahci_uc_priv *uc_priv, u16 *id)
{
...
static u32 iprop_ahsata_rw_cmd(struct ahci_uc_priv *uc_priv, u32 start,
...
static void iprop_ahsata_flush_cache(struct ahci_uc_priv *uc_priv)
{
	struct sata_fis_h2d h2d __aligned(ARCH_DMA_MINALIGN);
	struct sata_fis_h2d *cfis = &h2d;
	u8 port = uc_priv->hard_port_no;
...
static u32 iprop_ahsata_rw_cmd_ext(struct ahci_uc_priv *uc_priv, u32 start,
...
static void iprop_ahsata_flush_cache_ext(struct ahci_uc_priv *uc_priv)
{
	struct sata_fis_h2d h2d __aligned(ARCH_DMA_MINALIGN);
	struct sata_fis_h2d *cfis = &h2d;
	u8 port = uc_priv->hard_port_no;
...
static void iprop_ahsata_init_wcache(struct ahci_uc_priv *uc_priv, u16 *id)
{
				  ...
				  int is_write)
{
	...
	do {
		if (blks > max_blks) {
			if (max_blks != iprop_ahsata_rw_cmd_ext(uc_priv, start,
							      max_blks, addr,
							      is_write))
				return 0;
			start += max_blks;
			...
			if (blks != iprop_ahsata_rw_cmd_ext(uc_priv, start, blks,
							  addr, is_write))
				return 0;
			start += blks;
			blks = 0;
				  ...
				  int is_write)
{
	...
	do {
		if (blks > max_blks) {
			if (max_blks != iprop_ahsata_rw_cmd(uc_priv, start,
							  max_blks, addr,
							  is_write))
				return 0;
			start += max_blks;
			...
			if (blks != iprop_ahsata_rw_cmd(uc_priv, start, blks,
						      addr, is_write))
				return 0;
			start += blks;
			blks = 0;
...
static int iprop_ahci_start_ports(struct ahci_uc_priv *uc_priv)
{
	u32 linkmap;
	int i;
...
static int iprop_ahsata_scan_common(struct ahci_uc_priv *uc_priv,
				  struct blk_desc *pdev)
{
	...
	iprop_ahsata_identify(uc_priv, id);
	...
	iprop_ahsata_xfer_mode(uc_priv, id);

	/* Get the write cache status from device */
	iprop_ahsata_init_wcache(uc_priv, id);

	/* Set the xfer mode to highest speed */
	ahci_set_feature(uc_priv, port);

	iprop_ahsata_print_info(pdev);
			       ...
			       lbaint_t blkcnt, const void *buffer)
{
	...
	if (desc->lba48) {
		...
		if ((flags & SATA_FLAG_WCACHE) && (flags & SATA_FLAG_FLUSH_EXT))
			iprop_ahsata_flush_cache_ext(uc_priv);
	} else {
		rc = ata_low_level_rw_lba28(uc_priv, blknr, blkcnt, buffer,
					    WRITE_CMD);
		if ((flags & SATA_FLAG_WCACHE) && (flags & SATA_FLAG_FLUSH))
			iprop_ahsata_flush_cache(uc_priv);
	}
...
static int ahci_init_one(int pdev)
{
	...
	if (pdev == 0)
		uc_priv->mmio_base =
			(void __iomem *)PS2FPGA_SATA_CONFIG_OFFSET;
	else if (pdev == 1)
		uc_priv->mmio_base =
			(void __iomem *)PS2FPGA_SATA2_CONFIG_OFFSET;
...
int init_sata(int dev)
{
	...
	/*get environment max sata device*/
	env = env_get_ulong("sys_max_sata_dev", 10, CONFIG_SYS_SATA_MAX_DEVICE);
	...
	return iprop_ahci_start_ports(uc_priv) ? 1 : 0;
}
...
int scan_sata(int dev)
{
	...
	return iprop_ahsata_scan_common(uc_priv, pdev);
}
...
int iprop_ahsata_scan(struct udevice *dev)
{
	...
	*/
	...
	if (!blk) {
		ret = blk_create_devicef(dev, "iprop_ahsata_blk", "blk",
	...
	ret = iprop_ahsata_scan_common(uc_priv, desc);
	if (ret) {
...
int iprop_ahsata_probe(struct udevice *dev)
{
	...
	return iprop_ahci_start_ports(uc_priv);
}

static ulong iprop_ahsata_read(struct udevice *blk, lbaint_t blknr,
			     lbaint_t blkcnt, void *buffer)
{
...
static ulong iprop_ahsata_write(struct udevice *blk, lbaint_t blknr,
			      lbaint_t blkcnt, const void *buffer)
{
	struct blk_desc *desc = dev_get_uclass_platdata(blk);
	struct udevice *dev = dev_get_parent(blk);
...
static const struct blk_ops iprop_ahsata_blk_ops = {
	.read	= iprop_ahsata_read,
	.write	= iprop_ahsata_write,
};

U_BOOT_DRIVER(iprop_ahsata_blk) = {
	.name		= "iprop_ahsata_blk",
	.id		= UCLASS_BLK,
	.ops		= &iprop_ahsata_blk_ops,
};
